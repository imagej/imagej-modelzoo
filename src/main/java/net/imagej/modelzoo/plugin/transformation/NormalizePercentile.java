...
import io.bioimage.specification.transformation.preprocessing.NormalizePercentileTransformation;
...
@Plugin(type = NodeProcessor.class, name = NormalizePercentileTransformation.name)
public class NormalizePercentile
		extends DefaultImageNodeProcessor<NormalizePercentileTransformation> {
	...
	protected <I extends RealType<I> & NativeType<I>, O extends RealType<O> & NativeType<O>> ImageDataReference<FloatType> process(RandomAccessibleInterval<I> in, ImageDataReference<O> outType) {
		...
		float[] resValues = computePercentiles(in, new float[]{minPercentile, maxPercentile});
		...
		RandomAccessibleInterval<FloatType> converted = Converters.convert(in, converter, new FloatType());
	...
	@Override
	public void readSpecification(NormalizePercentileTransformation specification) {
	...
	public Number getLowerPercentile() {
		return minPercentile;
	}

	public Number getUpperPercentile() {
